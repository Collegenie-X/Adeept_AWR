...
"""
함수형 라인 추적 자율 주행차 (초음파 센서 장애물 회피 포함)
Functional Line Following Car with Ultrasonic Obstacle Avoidance
...
# 고급 회전 설정 (미세 조정용)
GENTLE_TURN_SPEED_RATIO = 0.7  # 부드러운 회전을 위한 속도 비율
SHARP_TURN_SPEED_RATIO = 1.2  # 급회전을 위한 속도 비율

# 장애물 회피 속도 설정 (후진 금지 - 좌회전→직진→우회전 패턴)
OBSTACLE_AVOIDANCE_LEFT_TURN_SPEED = 50  # 좌회전 회피 속도
OBSTACLE_AVOIDANCE_FORWARD_SPEED = 45  # 회피 중 직진 속도
OBSTACLE_AVOIDANCE_RIGHT_TURN_SPEED = 50  # 우회전 복귀 속도
OBSTACLE_AVOIDANCE_SLOW_DOWN_RATIO = 0.6  # 감속 비율 (0.0-1.0)

# 장애물 회피 시간 설정 (초)
OBSTACLE_AVOIDANCE_LEFT_TURN_TIME = 0.8  # 1단계: 좌회전 지속 시간
OBSTACLE_AVOIDANCE_FORWARD_TIME = 1.2  # 2단계: 직진 회피 지속 시간
OBSTACLE_AVOIDANCE_RIGHT_TURN_TIME = 0.8  # 3단계: 우회전 복귀 지속 시간
...
def print_current_settings():
    """현재 설정된 속도 값들을 출력한다"""
    print("\n" + "=" * 60)
    print("🚗 함수형 라인 추적 자율 주행차 설정")
    print("=" * 60)
    print(f"직진 속도: {FORWARD_SPEED}%")
    print(
        f"좌회전 시: 우측모터 {LEFT_TURN_RIGHT_MOTOR_SPEED}% (직진), 좌측모터 {LEFT_TURN_LEFT_MOTOR_SPEED}% (후진)"
    )
    print(
        f"우회전 시: 좌측모터 {RIGHT_TURN_LEFT_MOTOR_SPEED}% (직진), 우측모터 {RIGHT_TURN_RIGHT_MOTOR_SPEED}% (후진)"
    )
    ...
    print(
        f"회피 좌회전: {OBSTACLE_AVOIDANCE_LEFT_TURN_SPEED}% ({OBSTACLE_AVOIDANCE_LEFT_TURN_TIME}초)"
    )
    print(
        f"회피 직진: {OBSTACLE_AVOIDANCE_FORWARD_SPEED}% ({OBSTACLE_AVOIDANCE_FORWARD_TIME}초)"
    )
    print(
        f"회피 우회전: {OBSTACLE_AVOIDANCE_RIGHT_TURN_SPEED}% ({OBSTACLE_AVOIDANCE_RIGHT_TURN_TIME}초)"
    )
    print(f"감속 비율: {OBSTACLE_AVOIDANCE_SLOW_DOWN_RATIO}")
    print()
    print("🎯 적응형 회전 제어")
    print(f"부드러운 회전 비율: {GENTLE_TURN_SPEED_RATIO}")
    print(f"급회전 속도 비율: {SHARP_TURN_SPEED_RATIO}")
    print("라인 위치별 회전 강도: 0-0.3(부드러움), 0.3-0.7(일반), 0.7+(급회전)")
    print("=" * 60)
...
def execute_left_turn_with_intensity(intensity="normal"):
    """회전 강도를 조절하여 좌회전을 실행한다"""
    global statistics
    statistics["left_turn_count"] += 1

    # 강도에 따른 속도 조절
    if intensity == "gentle":
        right_speed = int(LEFT_TURN_RIGHT_MOTOR_SPEED * GENTLE_TURN_SPEED_RATIO)
        left_speed = int(LEFT_TURN_LEFT_MOTOR_SPEED * GENTLE_TURN_SPEED_RATIO)
        description = "부드러운 좌회전"
    elif intensity == "sharp":
        right_speed = int(LEFT_TURN_RIGHT_MOTOR_SPEED * SHARP_TURN_SPEED_RATIO)
        left_speed = int(LEFT_TURN_LEFT_MOTOR_SPEED * SHARP_TURN_SPEED_RATIO)
        description = "급좌회전"
    else:  # normal
        right_speed = LEFT_TURN_RIGHT_MOTOR_SPEED
        left_speed = LEFT_TURN_LEFT_MOTOR_SPEED
        description = "좌회전"

    set_motor_speeds(
        right_speed, left_speed, f"{description} (우측{right_speed}+좌측{left_speed})"
    )


def execute_right_turn_with_intensity(intensity="normal"):
    """회전 강도를 조절하여 우회전을 실행한다"""
    global statistics
    statistics["right_turn_count"] += 1

    # 강도에 따른 속도 조절
    if intensity == "gentle":
        right_speed = int(RIGHT_TURN_RIGHT_MOTOR_SPEED * GENTLE_TURN_SPEED_RATIO)
        left_speed = int(RIGHT_TURN_LEFT_MOTOR_SPEED * GENTLE_TURN_SPEED_RATIO)
        description = "부드러운 우회전"
    elif intensity == "sharp":
        right_speed = int(RIGHT_TURN_RIGHT_MOTOR_SPEED * SHARP_TURN_SPEED_RATIO)
        left_speed = int(RIGHT_TURN_LEFT_MOTOR_SPEED * SHARP_TURN_SPEED_RATIO)
        description = "급우회전"
    else:  # normal
        right_speed = RIGHT_TURN_RIGHT_MOTOR_SPEED
        left_speed = RIGHT_TURN_LEFT_MOTOR_SPEED
        description = "우회전"
...
def execute_avoidance_stage1_left_turn():
    ...
    set_motor_speeds(
        OBSTACLE_AVOIDANCE_LEFT_TURN_SPEED,
        -OBSTACLE_AVOIDANCE_LEFT_TURN_SPEED,
        "1단계: 좌회전 회피",
    )
...
def execute_avoidance_stage2_forward():
    ...
    set_motor_speeds(
        OBSTACLE_AVOIDANCE_FORWARD_SPEED,
        OBSTACLE_AVOIDANCE_FORWARD_SPEED,
        "2단계: 직진 회피",
    )
...
def execute_avoidance_stage3_right_turn():
    ...
    set_motor_speeds(
        -OBSTACLE_AVOIDANCE_RIGHT_TURN_SPEED,
        OBSTACLE_AVOIDANCE_RIGHT_TURN_SPEED,
        "3단계: 우회전 복귀",
    )
...
def process_avoidance_stages():
    ...
    if avoidance_stage == 1:  # 좌회전 단계
        ...
        if elapsed_time >= OBSTACLE_AVOIDANCE_LEFT_TURN_TIME:
            avoidance_stage = 2
        ...
        if elapsed_time >= OBSTACLE_AVOIDANCE_FORWARD_TIME:
            avoidance_stage = 3
            avoidance_start_time = current_time
            print("  → 3단계 시작: 우회전 복귀")
        ...
        if elapsed_time >= OBSTACLE_AVOIDANCE_RIGHT_TURN_TIME:
            # 회피 완료
            obstacle_avoidance_active = False
            avoidance_stage = 0
            print("✅ 장애물 회피 완료!")
...
def execute_line_following_drive(position, description):
    """감지된 라인 위치에 따라 주행한다 (적응형 회전 강도)"""
    if position == 0:
        # 중앙 - 직진
        drive_forward()
    elif position < 0:
        # 좌측으로 치우침 - 우회전 필요
        if abs(position) > 0.7:  # 급회전 필요
            execute_right_turn_with_intensity("sharp")
        elif abs(position) > 0.3:  # 일반 회전
            execute_right_turn()
        else:  # 부드러운 회전
            execute_right_turn_with_intensity("gentle")
    else:  # position > 0
        # 우측으로 치우침 - 좌회전 필요
        if abs(position) > 0.7:  # 급회전 필요
            execute_left_turn_with_intensity("sharp")
        elif abs(position) > 0.3:  # 일반 회전
            execute_left_turn()
        else:  # 부드러운 회전
            execute_left_turn_with_intensity("gentle")
...
def execute_obstacle_avoidance_control_logic(distance):
    ...
    if danger_level == "danger":
        ...
        reduced_speed = apply_speed_reduction(
            FORWARD_SPEED, OBSTACLE_AVOIDANCE_SLOW_DOWN_RATIO
        )
...
def print_help():
    ...
    print("  test_turn - 회전 동작 테스트")
    print("\n💡 속도 조절:")
    print("  파일 상단의 전역 상수를 수정하세요:")
    print("  - FORWARD_SPEED: 직진 속도")
    print("  - LEFT_TURN_*: 좌회전 속도 (우측직진+좌측후진)")
    print("  - RIGHT_TURN_*: 우회전 속도 (좌측직진+우측후진)")
    print("  - GENTLE_TURN_SPEED_RATIO: 부드러운 회전 비율")
    print("  - SHARP_TURN_SPEED_RATIO: 급회전 속도 비율")
    print("  - OBSTACLE_AVOIDANCE_*: 장애물 회피 속도")
...
def test_turn_movements():
    """회전 동작을 테스트한다"""
    print("\n🧪 회전 동작 테스트")
    print("다양한 강도의 회전 동작을 테스트합니다...")
...
def main():
    """메인 함수 - 프로그램의 진입점"""
    print("🚗 함수형 라인 센서 기반 자율 주행차 (장애물 회피 포함)")
    print("=" * 60)
