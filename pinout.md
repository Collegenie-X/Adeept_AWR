...
|------|-----------|---------|------|-----------|
| **모터 A Enable** | GPIO 26 | 37 | 우측 모터 PWM 제어 | 3.3V |
| **모터 A Pin1** | GPIO 19 | 35 | 우측 모터 방향 제어 1 | 3.3V |
| **모터 A Pin2** | GPIO 21 | 40 | 우측 모터 방향 제어 2 | 3.3V |
...
| **라인센서 우측** | GPIO 13 | 33 | 우측 라인 감지 | 3.3V |
...
```
라즈베리파이 핀        L298N 핀        기능
GPIO 26 (물리 37)  →   ENA           모터A Enable (PWM)
GPIO 19 (물리 35)  →   IN1           모터A 방향 제어 1
GPIO 21 (물리 40)  →   IN2           모터A 방향 제어 2
...
```
   ...
   GPIO 13   (33) (34) GND      ← 라인센서 우측
   GPIO 19   (35) (36) GPIO 16  ← 라인센서 중앙 / 모터A Pin1
   GPIO 26   (37) (38) GPIO 20  ← 라인센서 좌측 / 부저 / 모터A Enable
        GND  (39) (40) GPIO 21  ← 모터A Pin2

주요 핀 매핑:
- GPIO 26 (37): 모터A Enable
- GPIO 19 (35): 모터A Pin1
- GPIO 21 (40): 모터A Pin2
...
```
[라즈베리파이]
    ├── GPIO 26,19,21,17,27,18 → [L298N] → [4개 기어모터]
    ├── GPIO 11,8 → [HC-SR04 초음파센서]
    ├── GPIO 20,16,13 → [3개 라인센서]
...
| 구성요소 | GPIO | 물리핀 | 기능 | 상태 |
|---------|------|--------|------|------|
| **모터 A Enable** | 26 | 37 | 우측 모터 PWM | ✅ 확정 |
| **모터 A Pin1** | 19 | 35 | 우측 모터 방향1 | ✅ 확정 |
| **모터 A Pin2** | 21 | 40 | 우측 모터 방향2 | ✅ 확정 |
