...
def setup():
    """서비스 준비 (하드웨어 유무는 서비스 내부에서 처리)"""
    global line_service, motor_service, ultrasonic_service, keyboard_service
    ...
    try:
        ...
        # 상태 안내
        hw_line = getattr(line_service, "controller", None) is not None
        hw_motor = getattr(motor_service, "controller", None) is not None
        hw_ultra = getattr(ultrasonic_service, "sensor", None) is not None
        print(
...
def set_motor_speeds_quiet(right_speed: int, left_speed: int):
    """중복 설정을 피하며 모터 속도를 설정 (서비스에 위임)"""
    try:
        if not motor_service:
            return
        motor_service.set_speeds(right_speed, left_speed)
    except Exception:
        pass
...
def read_line():
    ...
    """
    if line_service and getattr(line_service, "controller", None):
        try:
            line_info = line_service.get_position_info()
...
def get_single_key():
    ...
    if not keyboard_service:
        return None
    return keyboard_service.read_single_key(0.02)
...
def get_line_input():
    ...
    if not keyboard_service:
        return None
    return keyboard_service.read_line()
    ...
    while running:
        try:
            ...
            # 속도 조절 키들 (즉시 반응)
                ...
                print("\n🔍 라인 센서 상태 확인 중 (3초)...")
                show_line_sensor_status(3)
            elif key == "z":
                print("\n🔍 거리 센서 상태 확인 중...")
                show_distance_sensor_status(5)
            elif key == "t":
                print("\n🧪 조향 테스트 시퀀스 시작")
...
def get_line_sensor_snapshot(prefix: str = "센서") -> str:
    ...
    try:
        if line_service and getattr(line_service, "controller", None):
            info = line_service.get_position_info()
...
def show_distance_sensor_status(TIMEOUT: int = 10):
    """거리 센서 상태 실시간 표시"""
    print("=" * 50)
    print("📏 거리 센서 상태 모니터링 ({}초간)".format(TIMEOUT))
    print("=" * 50)

    if ultrasonic_service and getattr(ultrasonic_service, "sensor", None):
        start_time = time.time()
        distances = []
        while time.time() - start_time < TIMEOUT:
            try:
                distance = ultrasonic_service.read_distance_cm()
                if distance:
...
def stop():
    """정지"""
    if motor_service and getattr(motor_service, "controller", None):
        motor_service.stop()
        print("Stop")
    else:
